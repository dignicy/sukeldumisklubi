	...
	<div class="container mx-auto px-6 relative z-10">
		<div class="max-w-7xl mx-auto">
			...
			{#if safariEvents.length > 0}
				<!-- Mobile Carousel (hidden on md+ screens) -->
				<div class="block md:hidden mb-16">
					...
					>
						<Carousel.Content class="-ml-4">
							{#each safariEvents as event}
								<Carousel.Item class="pl-4 basis-4/5">
									<Card class="group overflow-hidden rounded-2xl border border-white/20 shadow-none transition-all duration-300 hover:shadow-xl bg-white/85 backdrop-blur-sm h-full">
										<div class="relative h-48 overflow-hidden">
											...
											<div class="absolute left-4 top-4">
												<span class="rounded-full px-3 py-1 text-sm font-medium {getEventTypeStyle(event.type).bg} {getEventTypeStyle(event.type).text}">
													{isUpcoming(event.startDate) ? 'Tulemas' : 'Toimunud'}
												</span>
											</div>
											{#if event.price}
												<div class="absolute right-4 top-4 rounded-full bg-white/90 px-3 py-1 backdrop-blur-sm">
											...
											{#if getDaysUntilEvent(event.startDate) >= 0 && getDaysUntilEvent(event.startDate) <= 7}
												<div class="absolute bottom-4 left-4 rounded-md bg-red-500 px-2 py-1 text-xs font-medium text-white">
				...
				<div class="hidden md:grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
					{#each safariEvents as event}
						<Card class="group overflow-hidden rounded-2xl border border-white/20 shadow-none transition-all duration-300 hover:shadow-xl bg-white/85 backdrop-blur-sm">
							<div class="relative h-48 overflow-hidden">
								<img
									src={event.image}
									alt={event.title}
			...
			{:else}
				<!-- Fallback: No safari events available -->
				<div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
					<!-- Placeholder card 1 -->
					<div class="group relative">
						<div class="flex h-full flex-col overflow-hidden rounded-3xl border border-white/20 bg-white/95 backdrop-blur-sm transition-all duration-500 group-hover:scale-[1.02] group-hover:shadow-2xl">
							<div class="h-48 overflow-hidden bg-gray-300">
								<img
									src="/photos/sukeldumisklubi-photo-credit-manomi-tennakoon-2016.webp"
									alt="Sukeldumissafari"
									class="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105"
								/>
							</div>
							<div class="flex flex-grow flex-col p-8">
								<div class="mb-2">
									<span class="inline-block bg-purple-100 text-purple-600 text-xs font-semibold px-3 py-1 rounded-full">
										🌊 Sukeldumissafari
									</span>
								</div>
								<h3 class="mb-4 text-2xl font-bold text-gray-900">Põnevad reisid tulemas</h3>
								<p class="mb-8 leading-relaxed text-gray-600">
									Hetkel planeerime uusi põnevaid sukeldumissafarisid. 
									Jälgi meie ürituste kalendrit värskete värskenduste jaoks.
								</p>
								<Button class="mt-auto w-full rounded-xl bg-gray-900 hover:bg-gray-800">
